...
function bestfit = IICSO(fun_num,c,N)
    ...
    while(tot <=N)
            for i = 1 :Size
                ...
                for j = 1 : length(competitors)
                    ...
                    if epsiloncomper(fitness_p(:,competitors(j)),fitness_p(:,i),tot,N,e0) == 1
                            check = 1;
                            Velocity(n+1:3*n,i) = r1*Velocity(n+1:3*n,i) + r2*(Position(n+1:3*n,competitors(j))-Position(n+1:3*n,i))+r3*phi*(av_position(n+1:3*n) - Position(n+1:3*n,i)); 
                    end
                end
                if check == 1

                    SO1 = SO_Multiplication(Weight,SO(:,i));
                    SO2 = SO_Multiplication(c1*r1,SO_delete(Pbest_position(:,i),Position(:,i)));
                    SO3 =SO_Multiplication(c2*r2,SO_delete(Gbest_position,Position(:,i)));
                    SO4(:) = SO_add(SO1,SO_add(SO2,SO3));
                    SO(1:length(SO4),i) = SO4;
                             
                    Position(:,i) = SO_calcuate(Position(:,i),SO4(:));
                    ...
                    fitness_p(:,i)=Fitness(Pos,ship);
                     ...
                     if fitness_p(1,j)< fz(1,j) 
                            Pbest_position(:,j)=Pos;
                            fz(:,j)=fitness_p(:,j);
                     end
